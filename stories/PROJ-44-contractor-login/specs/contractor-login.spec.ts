...
import { VALID_IDS, INVALID_IDS } from '../fixtures/contractor-login.data';

const STORY = 'PROJ-44 | Contractor Login Via Nafath';

// ── Page load & UI elements ───────────────────────────────────────────────

test.describe(`${STORY} — Page elements @smoke @ui`, () => {

  let page: ContractorLoginPage;
  test.beforeEach(async ({ page: p }) => {
    page = new ContractorLoginPage(p);
    await page.goto();
  });

  test('TC-01 | Page loads at /contractor/login', async () => {
